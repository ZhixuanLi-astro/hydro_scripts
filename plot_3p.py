...
axs[0, 1].plot(rad, dust_2_vx1[0, 0, :]*UNIT_V, '-', lw=lwD['ms'], alpha=alpD['ms'], c = 'gray')
...
t_drift_2 = (rad*AU)/(dust_3_vx1[0, 0, :]*UNIT_V)/YR 

N1_column = dust_7_rho[0,0]/UNIT_L**2 
...
pp = zeros((2, len(rad)))
pp[0] = log10(dust_1_rho_xz[:, 0]/dust_2_rho_xz[:, 0])/log10(m_p[0,0]/m_p1[0,0])
pp[1] = log10(dust_2_rho_xz[:, 0]/dust_3_rho[0, 0])/log10(m_p1[0,0]/m_p2[0,0])

axs[1, 1].plot(rad, pp[0], marker='.', color='gray', label='p_01')
axs[1, 1].plot(rad, pp[1], marker='.', color='black', label='p_12')
...
mmin = 1.e-12
step = log(mmax[0,0,2:-2]/mmin)/3 
mDiv1 = mmin*exp(step )
mDiv2 = mmin*exp(2*step)
axs[0, 1].plot(rad_noghost, mDiv1, c ='gray', ls='--', label='m_div1')
axs[0, 1].plot(rad_noghost, mDiv2, c = 'lightgray', ls='--', label='m_div2')
